...
import {DocumentCreateModel, DocumentModel, DocumentsListFilters} from '../state/documents-dashboard.model';
...
export class DocumentsDashboardService {
  ...
  public createDocument(documentData: DocumentCreateModel): Observable<DocumentModel> {
  ...
  public updateDocument(documentId: DocumentModel['id'], updatedDocument: DocumentModel['name']): Observable<DocumentModel> {
    const url = `${this.API_ENDPOINT}/${documentId}/`;
    return this.http.patch<DocumentModel>(url, updatedDocument);
  }
  ...
  public changeDocumentStatus(documentId: DocumentModel['id']): Observable<void> {
